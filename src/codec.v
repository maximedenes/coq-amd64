...
Proof.
  admit.
  (*
  induction c => v l [/=EQ] /=; autorewrite with interp => /=.
(* Any *)
done.
(* Sym *)
...
+ split => //= H.
  ...
  simpl. split => //. *)
+ simpl. done.
+ simpl. admit. (*rewrite cAltSound 2!cSeqSound. rewrite interpAlt 2!interpSeq.
  split => H.
  destruct H.
  - destruct H as [l1 [l2 [H1 [H2 H3]]]]. subst.
...
+ simpl. rewrite cAltSound interpAlt.
  split => H.
  - destruct H. admit. (*left. by apply IHr1.
    right. by apply IHr2. *)
  - admit. (*destruct H. left. by apply IHr1.
    right. by apply IHr2. *)
+ simpl. admit. (* split => H. destruct H as [[x' xs] [H1 H2]].
