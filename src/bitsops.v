...
Fixpoint adcBmain n carry : BITS n -> BITS n -> BITS n.+1 :=
  if n is _.+1 then
    fun p1 p2 => let '(p1,b1) := splitlsb p1 in
           let '(p2,b2) := splitlsb p2 in
...
Definition sbbB {n} borrow (arg1 arg2: BITS n) :=
  let '(carry, res) := adcB (~~borrow) arg1 (invB arg2) in
  (~~carry, res).
...
Definition rorB {n} (p: BITS n.+1) : BITS n.+1 :=
  let '(p, b) := splitlsb p in joinmsb (b, p).

(* Rotate left: msb goes into lsb, everything else gets shifted left *)
Definition rolB {n} (p: BITS n.+1) :=
  let '(b, p) := splitmsb p in joinlsb (p, b).
